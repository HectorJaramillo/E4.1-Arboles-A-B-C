...
namespace Arbol_2
{
    public class Arbol
    {
        ...
        public void Impresion(Nodo nodo)//Imprime el digrama de todas las uniones
        {
            ...
            Altura = (y - 2) / 2;//Como el valor de y es 2 por eso le resto 2 y como cada vez que se le suman 2 significa que se le aumento una rama lo divido entre 2 me da la altura
            Nivel = Altura - 1;//La altura -1 me da el nivel
        }
        public void AlturayNivel()
        {
            Console.WriteLine("\nLa altura es: {0}", Altura);
            Console.WriteLine("El nivel es: {0}", Nivel);
            Console.WriteLine("La ruta al ultimo elemento es G-->A-->(B,C,D)");
        }
    }
}
